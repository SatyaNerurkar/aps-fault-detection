...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        """
        Description: This function will drop column which contains missing value more than specified threshold
        ...
        try:
            ...
            # Prepare the null vaues count report
            null_report = df.isna().sum()/df.shape[0]

            # selecting column name which contains null values greater than threshold
            logging.info(f"selecting column name which contains null above to {threshold}")
            drop_column_names = null_report[null_report>threshold].index
            logging.info(f"Columns to drop: {list(drop_column_names)}")

            # appending all the dropped column name in the validation error dictionary to generate final report.
    ...
    def is_required_columns_exists(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str)->bool:
        ...
        try:
            ...
            missing_columns = []
            # iterating through each column from the base dataframe (reference)
            ...
            # appending all the missing column name in the validation error dictionary to generate final report.
    ...
    def data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str):
        """
        Description: This function will validate whether all features have same data distribution or not.
        ====================================================================================
        base df: Accepts a pandas dataframe base df as reference
        current df: Accepts a pandas dataframe base df to be validated
        """
        try:
            ...
            for base_column in base_columns:
                ...
                logging.info(f"Hypothesis {current_data}")
                same_distribution =ks_2samp(base_data,current_data)

                # Generating data distribution report
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        ...
        try:
            ...
            # Replace "na" values to null from base dataframe (reference dataframe)
            base_df.replace({"na":np.NAN},inplace=True)
            logging.info(f"Replace na value in base df")

            # Drop null values colums from base df
            ...
            # All column are available in train df hence detecting "data drift"
            if train_df_columns_status:
                logging.info(f"As all column are available in train df hence detecting data drift")
                self.data_drift(base_df=base_df, current_df=train_df,report_key_name="data_drift_within_train_dataset")

            # All column are available in test df hence detecting "data drift"
